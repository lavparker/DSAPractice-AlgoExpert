...
function classPhotos(redShirtHeights, blueShirtHeights) {
  let teeeff = []
  sortRed = redShirtHeights.sort((a, b) => b - a); 
  sortBlue = blueShirtHeights.sort((a, b) => b - a); 
  for(let i = 0; i < sortRed.length; i++){
    let redS = sortRed[i]; 
    let blueS = sortBlue[i]; 
    teeeff.push(redS < blueS); 
  }

  if(teeeff.all === true || teeeff.all === false){
    return true
  }else{
    return false; 
  }
